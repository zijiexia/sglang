...
### Cap the decode CUDA graphs

Every cell above passes `--cuda-graph-max-bs-decode 64` wherever DSpark is on, and the H200 cells pass it on both recipes. Without it the derived batch list runs out of memory while capturing the decode graphs and the server never finishes starting — the failure is `Capture cuda graph failed: CUDA out of memory` from `decode_cuda_graph_runner`, not a runtime error you would see under load. If you assemble a command by hand rather than from the panel, carry that flag over. H200, the tightest board at 140 GiB, additionally needs `--mem-fraction-static 0.8`.
